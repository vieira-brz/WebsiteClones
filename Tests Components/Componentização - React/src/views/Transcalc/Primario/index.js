...
function Primario() {
  ...
  return (
    <div className='primario'>
      ...
      <TitleSection text='Primários' icon={<TollIcon />} />
      ...
      <div className='fields'>
        ...
        {rows.map((row, index) => (
          ...
          <div key={row.id} className='row'>
            ...
            <h4 id={`prim${row.id}-input`}>{`P${row.id}`}</h4>

            <TextField
              id={`tensao-prim${row.id}-input`}
              label='Tensão (V)'
              defaultValue={row.tensao}
              variant='outlined'
              type='search'
              onKeyUp={() => keyup_number(`tensao-prim${row.id}-input`)}
              onFocus={() => {
                clearOnFocus(index);
                clear_on_focus(`tensao-prim${row.id}-input`);
              }}
            />

            <TextField
              id={`corrente-prim${row.id}-input`}
              label='Corrente (A)'
            ...
            <TextField
              id={`calculado-prim${row.id}-input`}
              label='Fio calculado'
            ...
            <Box>
              <FormControl fullWidth>
                <InputLabel id={`diametro-prim${row.id}-select-label`}>Fio</InputLabel>
                <Select
                  labelId={`diametro-prim${row.id}-select-label`}
                  id={`diametro-prim${row.id}-select`}
                  label='Fio'
                  value={row.diametro}
        ...
        <div className='actions'>
          <DynamicRows rows={rows} setRows={setRows} objetoPadrao={objetoPadrao} auxiliarText="primários" limit='10'/>
        </div>
